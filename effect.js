...
$("document").ready(function () {
  ...
  $("#story").click(function () {
    ...
    const msgLen = $(".message p").length;
    const contentLen = $(".content p").length;
    $(".message").fadeIn("slow");
    $(".content").fadeIn("slow");
    function msgLoop(i, len, cls) {
      $(`${cls} p:nth-child(${i})`)
        .fadeOut("slow")
        .delay(800)
        .promise()
        .done(function () {
          i = i + 1;
          $(`${cls} p:nth-child(${i})`).fadeIn("slow").delay(1000);
          if (i == len) {
            $(`${cls} p:nth-child(${len - 1})`)
              .fadeOut("slow")
              .promise()
              .done(function () {
                if (cls !== ".content") {
                  msgLoop(0, contentLen, ".content");
                } else {
                  $(".cake").fadeIn("fast");
                }
              });
          } else {
            msgLoop(i, msgLen, cls);
          }
        });
      // body...
    }

    msgLoop(0, msgLen, ".message");
  });
});
